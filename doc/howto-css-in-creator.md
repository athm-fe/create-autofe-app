...
我大概是在 2016 年萌生开发 Creator 的想法，当初做过一些调研，那时候的 Webpack 还没有那么强大，也没有现在那么大的生态，针对 React 等开发确实比较强大，但是还不太适用于传统的前端开发方式。印象中，遇到的问题有：
...
1. CSS 的所在的目录结构丢失了，我们期望的是 `build/home/index.css`
...
第二个问题，找到一个插件来解决，远离是找到只有一个 CSS Entry 的 JS Asset，并删除它，配置如下：
...
1. Loader 配置好 SourceMap，有的 Loader 会智能的处理 Loader 处理链上的 SourceMap，而有的需要你手动配置开关。
...
1. 凡事带 `eval` 的，打包速度比较快，但是不能配合 `mini-css-extract-plugin` 使用，可以配合 `style-loader` 使用，Creator 用不了
