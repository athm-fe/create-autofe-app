...
module.exports = function prepareProxy (proxy, appPublicFolder) {
  ...
  if (Array.isArray(proxy) || (typeof proxy !== 'object' && typeof proxy !== 'string')) {
    console.log(
      chalk.red(
        'When specified, "proxy" must be a string or an object.'
      )
    )
    console.log(
    ...
    console.log(
      chalk.red(
        'Either remove "proxy", or make it an object.'
      )
    )
    process.exit(1)
  }
  ...
  if (typeof proxy === 'string') {
    if (!/^http(s)?:\/\//.test(proxy)) {
      console.log(
        chalk.red(
          'When "proxy" is specified it must start with either http:// or https://'
        )
      )
      process.exit(1)
    }
  ...
  return Object.keys(proxy).map(context => {
    ...
    if (!config.hasOwnProperty('target')) {
      console.log(
        chalk.red(
          'When `proxy` is an object, each `context` object must have a ' +
