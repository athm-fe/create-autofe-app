...
function serveIndex(options) {
  ...
  var contentBase = opts.contentBase;
  ...
  return function (req, res, next) {
    ...
    const done = (list) => {
      state.count--;
      if (list) {
        list.forEach(itemSrc => {
          let isExist = false;
          state.fileList.forEach((item) => {
            if (itemSrc.name === item.name) {
              isExist = true;
            }
          });
          if (!isExist) {
            state.fileList.push(itemSrc);
          }
        });
      }
      ...
      let fileList = Array.from(state.fileList);

      if (!hidden) {
        fileList = removeHidden(fileList);
      }
      if (filter) {
        fileList = fileList.filter(function(file, index, list) {
          return filter(file, index, list);
        });
      }
      ...
      serveIndex[mediaType[type]](req, res, fileList, next, originalDir, showUp, icons, view, template, stylesheet);
    };

    try {
      var outputPath = middleware.getFilenameFromUrl(req.url);

      if (!filesystem.statSync(outputPath).isDirectory()) {
        return done();
      }

      state.isDirectory = true;

      const list = filesystem.readdirSync(outputPath);
    ...
    contentBase.forEach((basePath) => {
      // resolve root to absolute and normalize
      var rootPath = normalize(resolve(basePath) + sep);
      ...
      fs.stat(path, function(err, stat){
        ...
        fs.readdir(path, function(err, files){
          ...
          // TODO: list 里每一项是文件 or 文件夹
          return done(files.map(item => ({
            name: item,
            dir: path,
          })));
        });
...
serveIndex.html = function _html(req, res, files, next, dir, showUp, icons, view, template, stylesheet) {
  var render = typeof template !== 'function'
    ? createHtmlRender(template)
    : template

  // stat all files
  stat(files, function (err, fileList) {
    ...
    if (showUp) {
      fileList.unshift({ name: '..', isDir: true });
    }
...
serveIndex.json = function _json (req, res, files, next) {
  // stat all files
  stat(files, function (err, fileList) {
    if (err) return next(err)
...
serveIndex.plain = function _plain (req, res, files, next) {
  // stat all files
  stat(files, function (err, fileList) {
    if (err) return next(err)
...
function stat(files, cb) {
  ...
  files.forEach(function(file){
    var filesystem = file.filesystem || fs;
    batch.push(function(done){
      filesystem.stat(join(file.dir, file.name), function(err, stat){
        if (err && err.code !== 'ENOENT') return done(err);

        // pass ENOENT as null stat, not error
        done(null, {
          name: file.name,
          isDir: stat ? stat.isDirectory() : null,
        })
      });
    });
  });
